...
def power_usage():
    ...
    for i in range(profile_time_1_day_mission['afternoon_rest']+2):
        power_series_1_day.append(power_usage_baseline)
    
    return power_series_1_day

power_used = power_usage()
net_power = solar_power - power_used #Power that is directly charging/discharging the battery

plt.plot(interpolated_local_time,net_power)
plt.title('Net power available to charge/discharge the battery over a Martian day')
plt.xlabel('Mission timeline [s]')
plt.ylabel('Net power [W]')
plt.show()

timestep = 1 #Use timestep of 1 second to get joules
energy_state = np.cumsum(net_power*timestep) #Cumulative energy state

plt.plot(interpolated_local_time, energy_state)
plt.title('Energy state over a full mission')
plt.xlabel('Mission timeline [s]')
plt.ylabel('Energy state [J]')
plt.show()

min_energy = np.min(energy_state)
max_energy = np.max(energy_state)
battery_capacity = max_energy - min_energy
print('battery capacity [Wh]', battery_capacity/3600)
